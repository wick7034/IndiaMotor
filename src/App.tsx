...
const Speedometer = ({ speed }: { speed: number }) => {
  ...
  return (
    <div className="relative w-64 h-64 md:w-80 md:h-80 flex items-center justify-center">
      {/* Outer Ring */}
      <div className="absolute inset-0 rounded-full border-4 border-border-main border-t-neon-yellow/50 rotate-[-45deg]"></div>
      ...
      <div className="absolute inset-0">
        {[...Array(19)].map((_, i) => (
          ...
          >
            <div className={`w-0.5 h-3 ${i % 3 === 0 ? 'bg-neon-yellow h-5' : 'bg-text-muted/20'}`}></div>
          </div>
        ))}
      </div>
      ...
      <div className="absolute bottom-12 text-center">
        ...
        <div className="text-[10px] uppercase tracking-widest text-text-muted font-semibold">KM/H</div>
      </div>

      {/* Indicators */}
      <div className="absolute bottom-4 flex gap-8 text-[10px] font-mono text-text-muted">
        <div className="flex flex-col items-center">
          <div className="w-12 h-1 bg-bg-muted rounded-full overflow-hidden mb-1">
            <motion.div 
      ...
      <div className="w-1/2 text-right">
        {index % 2 === 0 && (
          <>
            ...
            <p className="text-text-muted text-sm max-w-xs ml-auto">{description}</p>
          </>
        )}
      </div>
      
      <div className="relative flex flex-col items-center">
        <div className="w-1 bg-bg-muted h-32 absolute top-0 -z-10"></div>
        <div className="w-12 h-12 rounded-lg bg-neon-yellow flex items-center justify-center shadow-[0_0_20px_rgba(var(--neon-yellow-rgb),0.4)] z-10">
          <span className="font-bold text-[#050505]">0{index + 1}</span>
        </div>
      </div>
...
const TestimonialCard = ({ name, review, rating }: { name: string, review: string, rating: number }) => {
  return (
    <motion.div 
      whileHover={{ y: -5 }}
      className="glass-card p-8 rounded-3xl border border-border-main"
    >
      ...
      <p className="text-text-main/70 italic mb-6 leading-relaxed">"{review}"</p>
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-bg-muted flex items-center justify-center font-bold text-neon-yellow">
          {name.charAt(0)}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="mb-10 relative z-10">
        <div className="text-xs uppercase tracking-widest text-text-muted mb-1 font-bold">{type}</div>
        <div className="text-4xl font-bold text-text-main">₹{price}</div>
      </div>
      ...
      <ul className="space-y-4 mb-8 relative z-10">
        {features.map((f: string, i: number) => (
          <li key={i} className="flex items-center gap-3 text-sm text-text-main/70">
      ...
      <button className="w-full py-4 rounded-xl bg-bg-muted border border-border-main text-sm font-bold hover:bg-text-main hover:text-dark-bg transition-all duration-300">
        SELECT RIDE
      </button>
    </motion.div>
  );
...
const SteeringWheel = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-12">
      <div className="relative w-64 h-32 bg-bg-muted rounded-t-full border-t border-border-main flex items-end justify-center overflow-hidden">
        <motion.div 
          className="w-12 h-20 bg-neon-yellow rounded-lg shadow-[0_0_30px_rgba(var(--neon-yellow-rgb),0.5)]"
          animate={{ x: rotation / 5, rotate: rotation / 10 }}
        >
          <div className="w-full h-1 bg-text-main/20 mt-2"></div>
          <div className="w-full h-1 bg-text-main/20 mt-2"></div>
        </motion.div>
      </div>
      ...
      >
        <motion.div 
          className="w-full h-full rounded-full border-[16px] border-border-main flex items-center justify-center relative"
          animate={{ rotate: rotation }}
          transition={{ type: 'spring', stiffness: 100, damping: 20 }}
        >
          {/* Spokes */}
          <div className="absolute w-full h-4 bg-border-main"></div>
          <div className="absolute w-4 h-1/2 bg-border-main top-1/2 -translate-y-1/2"></div>
          
          {/* Center */}
          <div className="w-24 h-24 rounded-full bg-card-bg border-4 border-border-main flex items-center justify-center shadow-inner">
            <div className="text-text-muted font-bold tracking-widest text-[10px]">DRIVE</div>
          </div>
        ...
        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] text-text-muted uppercase tracking-widest font-bold flex items-center gap-2">
    ...
    <div className="min-h-screen bg-dark-bg selection:bg-neon-yellow selection:text-dark-bg overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 px-6 py-6 md:px-12 flex justify-between items-center bg-gradient-to-b from-dark-bg to-transparent">
        <div className="flex items-center gap-2 group cursor-pointer">
          <div className="w-10 h-10 rounded-lg bg-neon-yellow flex items-center justify-center shadow-[0_0_15px_rgba(var(--neon-yellow-rgb),0.4)] transition-transform group-hover:rotate-12">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-semibold text-text-muted">
          <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-bg-muted border border-border-main hover:bg-text-main hover:text-dark-bg transition-all" title="Instagram">
            <Instagram size={20} />
          </a>
          <a href="https://wa.me/919876543210" target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-bg-muted border border-border-main hover:bg-text-main hover:text-dark-bg transition-all" title="WhatsApp">
            <MessageCircle size={20} />
          </a>
          <a href="#booking" className="ml-4 px-6 py-2 rounded-full bg-bg-muted border border-border-main hover:bg-text-main hover:text-dark-bg transition-all">BOOK NOW</a>
        </div>
      ...
      <section className="relative min-h-screen flex flex-col md:flex-row items-center justify-center px-6 md:px-24 pt-24 overflow-hidden">
        ...
        <div className="w-full md:w-1/2 z-10 text-center md:text-left mb-12 md:mb-0">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-text-muted max-w-md mb-10 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-center md:justify-start">
              <button 
                ...
                className={`group relative px-10 py-5 rounded-2xl font-bold text-lg transition-all duration-500 overflow-hidden ${isEngineStarted ? 'bg-neon-green text-dark-bg' : 'bg-neon-yellow text-dark-bg shadow-[0_0_30px_rgba(var(--neon-yellow-rgb),0.3)]'}`}
              >
                <div className="relative z-10 flex items-center gap-3">
                  {isEngineStarted ? 'ENGINE RUNNING' : 'START ENGINE'}
                  <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </div>
                <div className="absolute inset-0 bg-text-main/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
              </button>
              <div className="flex items-center gap-4 text-text-muted text-sm font-semibold">
                <div className="flex -space-x-2">
                  {[1,2,3].map(i => (
                    <div key={i} className="w-8 h-8 rounded-full border-2 border-dark-bg bg-bg-muted overflow-hidden">
      ...
      <section id="journey" className="py-32 relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-center mb-24 relative">
          {/* Ribbon Title */}
          <div className="w-full md:w-1/2 bg-neon-yellow py-10 px-6 md:pl-24 relative z-10 shadow-[0_10px_40px_rgba(var(--neon-yellow-rgb),0.2)]">
            <motion.div
          ...
          <div className="w-full md:w-1/2 h-48 relative flex items-center justify-center overflow-hidden bg-bg-muted border-y border-border-main mt-8 md:mt-0">
            ...
            <div className="absolute inset-0 flex flex-col justify-center gap-16 overflow-hidden">
              <motion.div 
                ...
                className="h-0.5 w-[200%] border-t-2 border-dashed border-border-main"
              ></motion.div>
              <motion.div 
                animate={{ x: ["-50%", "0%"] }}
                transition={{ 
            ...
            <div className="relative flex items-center gap-6 z-10">
              ...
              >
                ...
                <Car size={100} className="text-neon-yellow drop-shadow-[0_0_20px_rgba(var(--neon-yellow-rgb),0.6)] scale-x-[-1]" strokeWidth={1.5} />
              ...
              <div className="flex flex-col">
                ...
                <span className="text-text-muted font-bold text-[10px] tracking-[0.4em]">Rewari</span>
              </div>
            </div>
            ...
            {[...Array(8)].map((_, i) => (
              <motion.div
                ...
                className="absolute h-[1px] w-16 bg-text-muted/20"
              />
            ))}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto relative px-6 md:px-0">
          {/* Central Road Line */}
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-1 bg-bg-muted border-x border-dashed border-border-main hidden md:block"></div>
      ...
      {/* Packages Section */}
      <section id="packages" className="py-32 px-6 md:px-24 bg-bg-muted/20">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">Our Training Package</h2>
          <p className="text-text-muted">A single, comprehensive course designed to make you a pro.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-20 max-w-6xl mx-auto">
          <div className="w-full md:w-1/2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 rounded-2xl bg-bg-muted border border-border-main">
                <div className="text-neon-yellow mb-1 font-bold text-xl">98%</div>
                <div className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Pass Rate</div>
              </div>
              <div className="p-5 rounded-2xl bg-bg-muted border border-border-main">
                <div className="text-neon-green mb-1 font-bold text-xl">12k+</div>
                <div className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Lessons Done</div>
              </div>
              <div className="p-5 rounded-2xl bg-bg-muted border border-border-main flex items-center gap-4 group hover:bg-text-main/10 transition-colors">
                <Shield className="text-neon-yellow shrink-0 group-hover:scale-110 transition-transform" size={20} />
                <div className="text-[10px] text-text-muted uppercase tracking-widest font-bold leading-tight">Best Safety<br/>Measures</div>
              </div>
              <div className="p-5 rounded-2xl bg-bg-muted border border-border-main flex items-center gap-4 group hover:bg-text-main/10 transition-colors">
                <User className="text-neon-green shrink-0 group-hover:scale-110 transition-transform" size={20} />
                <div className="text-[10px] text-text-muted uppercase tracking-widest font-bold leading-tight">Experienced<br/>Instructors</div>
              </div>
              <div className="p-5 rounded-2xl bg-bg-muted border border-border-main flex items-center gap-4 group hover:bg-text-main/10 transition-colors col-span-2">
                <Zap className="text-neon-yellow shrink-0 group-hover:scale-110 transition-transform" size={20} />
                <div className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Affordable Fee Structure</div>
              </div>
            </div>
          </div>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none">
          <div className="w-full h-96 bg-bg-muted skew-y-[-1deg]"></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-24 relative">
          ...
          <div className="relative">
            ...
            <div className="absolute top-1/2 left-0 w-full h-0.5 bg-bg-muted -translate-y-1/2 hidden md:block z-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-0 relative z-10">
              <motion.div 
                whileHover={{ y: -10 }}
                className="bg-bg-muted border border-border-main p-10 md:border-r-0 md:rounded-l-3xl backdrop-blur-sm relative group overflow-hidden"
              >
              ...
              <motion.div 
                whileHover={{ y: -10 }}
                className="bg-bg-muted border border-border-main p-10 md:border-x-0 backdrop-blur-sm relative group overflow-hidden"
              >
              ...
              <motion.div 
                whileHover={{ y: -10 }}
                className="bg-bg-muted border border-border-main p-10 md:border-l-0 md:rounded-r-3xl backdrop-blur-sm relative group overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-full h-1 bg-neon-yellow scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500"></div>
                <div className="w-16 h-16 rounded-2xl bg-neon-yellow/10 flex items-center justify-center mb-8 group-hover:bg-neon-yellow/20 transition-colors">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="max-w-xl">
              ...
              <p className="text-text-muted">Real feedback from confident drivers who started their journey with us.</p>
            </div>
      ...
      <section id="booking" className="py-32 px-6 md:px-24">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-text-muted">Fill out your application and book your first lesson today.</p>
          </div>
          ...
          <div className="glass-card p-10 md:p-16 rounded-[40px] relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-12 border-b border-border-main pb-8">
              ...
              <div className="w-20 h-24 bg-bg-muted border border-border-main rounded-lg flex items-center justify-center">
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-text-muted font-bold ml-1">Full Name</label>
                <input type="text" placeholder="John Doe" className="w-full bg-bg-muted border border-border-main rounded-xl px-6 py-4 focus:outline-none focus:border-neon-yellow transition-colors" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-text-muted font-bold ml-1">Phone Number</label>
                <input type="tel" placeholder="+1 (555) 000-0000" className="w-full bg-bg-muted border border-border-main rounded-xl px-6 py-4 focus:outline-none focus:border-neon-yellow transition-colors" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-text-muted font-bold ml-1">Experience Level</label>
                <select className="w-full bg-bg-muted border border-border-main rounded-xl px-6 py-4 focus:outline-none focus:border-neon-yellow transition-colors appearance-none">
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-text-muted font-bold ml-1">Preferred Time</label>
                <select className="w-full bg-bg-muted border border-border-main rounded-xl px-6 py-4 focus:outline-none focus:border-neon-yellow transition-colors appearance-none">
              ...
              <div className="md:col-span-2 pt-4">
                <button className="w-full py-6 rounded-2xl bg-neon-yellow text-dark-bg font-bold text-lg shadow-[0_0_30px_rgba(var(--neon-yellow-rgb),0.3)] hover:scale-[1.02] transition-transform active:scale-95">
      ...
      {/* Footer */}
      <footer className="py-20 px-6 md:px-24 border-t border-border-main bg-dark-bg">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-neon-yellow flex items-center justify-center">
                <Car className="text-[#050505]" size={16} />
              </div>
              <span className="text-lg font-bold tracking-tighter">INDIA<span className="text-neon-yellow"> MOTOR</span></span>
            </div>
            <p className="text-text-muted text-sm">कार चलाना सीखिये</p>
            <p className="text-[10px] text-text-muted/50 mt-2 max-w-[200px]">Rewari, Haryana 123401 | +91 94663 85062</p>
          </div>

          <div className="flex flex-col items-center gap-4">
            <div className="w-32 h-1 bg-bg-muted rounded-full overflow-hidden">
              <motion.div 
            ...
            <div className="text-[10px] text-text-muted uppercase tracking-[0.3em] font-bold">System Status: Optimal</div>
            
            {/* Theme Toggle */}
            <div className="flex items-center gap-3 mt-4 p-1 rounded-full bg-bg-muted border border-border-main">
              <button 
                onClick={() => setIsDarkMode(true)}
                className={`p-2 rounded-full transition-all ${isDarkMode ? 'bg-neon-yellow text-[#050505]' : 'text-text-muted hover:text-text-main'}`}
              >
                <Moon size={14} />
              </button>
              <button 
          ...
          <div className="flex flex-col items-center md:items-end gap-4 text-center md:text-right">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-text-muted font-bold mb-1">Working Hours</p>
              <p className="text-sm text-text-main/70">Mon - Sat: 6:00 AM - 7:00 PM</p>
              <p className="text-[10px] text-red-500/60 font-bold uppercase tracking-tighter">Sunday: Closed</p>
            </div>
            <a 
              href="tel:+919466385062" 
              className="px-6 py-2 rounded-xl bg-neon-green/10 border border-neon-green/20 text-neon-green text-sm font-bold hover:bg-neon-green hover:text-[#050505] transition-all flex items-center gap-2"
            >
        ...
        <div className="text-center mt-20 text-[10px] text-text-muted/10 uppercase tracking-widest font-bold">
